/**
 * Smoke CSV PE · formato Director (DEPOSITO cabecera · Cant. Pares · ambas cols = bruto tier).
 */
import {
...
// LPC03: bruto tier = precio_unit 112000 (no neto 75600)
const lpc03Row = {
  cliente_id: "1",
  plazo_id: "1",
  pedido_id: null,
...
const lpc03 = resolvePreciosLineaPeCsv(lpc03Row, 3);
assert(lpc03.bruto === "112000", `LPC03 bruto=${lpc03.bruto}`);
assert(lpc03.neto === lpc03.bruto, `ambas cols iguales bruto=${lpc03.neto}`);

// LPN: precio_unit contaminado (= neto) → recuperar desde PPD LPN
const lpnContaminado = resolveBrutoLineaPeCsv(
  {
    ...lpc03Row,
    lista_precio_id: "1",
    precio_unit: "151560",
    precio_neto: "151560",
    ppd_precio_lpn: "168400",
    ppd_precio_lpc03: "112000",
    fid_id: 100,
  },
  1,
);
assert(lpnContaminado === 168400, `LPN bruto recuperado=${lpnContaminado}`);

// LPC03 elegido pero precio_unit = LPN → debe ir LPC03, nunca LPN
const lpc03AntiLpn = resolveBrutoLineaPeCsv(
  {
    ...lpc03Row,
    precio_unit: "100000",
    precio_neto: "75600",
...
assert(lpc03AntiLpn === 112000, `LPC03 anti-LPN=${lpc03AntiLpn}`);
...
assert(r1[13] === r1[14], `ambas cols precio iguales=${r1[13]}`);
assert(r1[13] === "112000", `bruto tier=${r1[13]}`);
...
const badCsv = [{ ...okCsv[0], precio_sin_descuento: "100000", precio_con_descuento: "100000" }];
...
console.log("OK smoke csv pe · ambas cols bruto tier Carlos");
console.log(lines[1]);
