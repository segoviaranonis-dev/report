-- MIG-163 · Alejandro Magno — campos comerciales SDRM en PPD (molécula PE)
-- cadena_comercial · es_liquidacion · temporada (638 col G) · cod_grupo
-- Doc: report/docs/MAPA_SDRM_ALEJANDRO_MAGNO_LIQUIDACION.md

ALTER TABLE public.pedido_proveedor_detalle
  ADD COLUMN IF NOT EXISTS am_cadena_comercial text,
  ADD COLUMN IF NOT EXISTS am_es_liquidacion boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS am_temporada text,
  ADD COLUMN IF NOT EXISTS am_cod_grupo text;

COMMENT ON COLUMN public.pedido_proveedor_detalle.am_cadena_comercial IS
  'Alejandro Magno · SDRM cadena comercial (LIQUIDACION|REGULAR|PROMO…) · filtro Panel PE';
COMMENT ON COLUMN public.pedido_proveedor_detalle.am_es_liquidacion IS
  'Alejandro Magno · flag LIQUIDACIÓN derivado del xlsx SDRM (654 col G · 638 col H)';
COMMENT ON COLUMN public.pedido_proveedor_detalle.am_temporada IS
  'Alejandro Magno · Kyly 638 · TEMPORADA Excel col G (VERANO|INVIERNO)';
COMMENT ON COLUMN public.pedido_proveedor_detalle.am_cod_grupo IS
  'Alejandro Magno · COD.GRUPO SDRM para trazabilidad comercial';

CREATE INDEX IF NOT EXISTS idx_ppd_am_liquidacion
  ON public.pedido_proveedor_detalle (am_es_liquidacion)
  WHERE am_es_liquidacion = true;

CREATE INDEX IF NOT EXISTS idx_ppd_am_cadena
  ON public.pedido_proveedor_detalle (am_cadena_comercial)
  WHERE am_cadena_comercial IS NOT NULL;

-- Vista PE — prioriza columnas AM en PPD, luego join SDRM
...
SELECT
  ...
  CASE
    WHEN pp.proveedor_importacion_id = 638
     ...
     AND NULLIF(
       regexp_replace(COALESCE(pe_stg.excel_color_code, col_j.nombre, ''), '^[Kk]', ''),
       ''
     ) IS NOT NULL
    THEN 'https://extrlcvcgypwazxipvqm.supabase.co/storage/v1/object/public/productos/'
         || btrim(ppd.linea) || '_'
         || regexp_replace(COALESCE(pe_stg.excel_color_code, col_j.nombre), '^[Kk]', '') || '.jpg'
  ...
  CASE
    ...
    ELSE 'CALZADO'::text
  END AS ramo_tipo,
  COALESCE(ppd.am_cod_grupo, sac.cod_grupo, cg.cod_grupo, pe_stg.cod_grupo) AS cod_grupo,
  COALESCE(sac.marca, cg.marca) AS sdrm_marca,
  COALESCE(sac.tipo0, cg.tipo0) AS sdrm_tipo0,
  COALESCE(sac.tipo1, cg.tipo1) AS sdrm_tipo1,
  COALESCE(sac.tipo2, cg.tipo2) AS sdrm_tipo2,
  CASE
  ...
  COALESCE(sac.es_promo, cg.es_promo, false) AS es_promo,
  COALESCE(
    ppd.am_cadena_comercial,
    sac.cadena_comercial,
    cg.cadena_comercial,
    'REGULAR'::text
  ) AS cadena_comercial,
  COALESCE(
...
COMMENT ON VIEW public.v_stock_pe_rimec IS
  'Pronta entrega RIMEC · PPD Alejandro Magno · MIG-163 AM comercial + pilares FK';
