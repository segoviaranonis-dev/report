-- MIG-191 · v_stock_pe_rimec rápida — DPE desde am_* (sin hash sac 12k)
-- Causa: count vista ~19s · página ordenada ~1.4s (Seq Scan sdrm_articulo_comercial).
-- Tras import Node PE, am_cod_grupo / am_cadena_comercial están llenos.
...
SELECT
  ...
  CASE
    ...
    ELSE 'CALZADO'::text
  END AS ramo_tipo,
  COALESCE(ppd.am_cod_grupo, pe_stg.cod_grupo, cg.cod_grupo) AS cod_grupo,
  cg.marca AS sdrm_marca,
  cg.tipo0 AS sdrm_tipo0,
  cg.tipo1 AS sdrm_tipo1,
  cg.tipo2 AS sdrm_tipo2,
  CASE
    WHEN ppd.am_cadena_comercial IS NOT NULL THEN COALESCE(ppd.am_es_liquidacion, false)
    ELSE COALESCE(cg.es_liquidacion, false)
  END AS es_liquidacion,
  CASE
    WHEN COALESCE(ppd.am_cadena_comercial, cg.cadena_comercial, 'REGULAR') = 'PROMOCIONAL'
    THEN true
    ELSE COALESCE(cg.es_promo, false)
  END AS es_promo,
  COALESCE(
...
LEFT JOIN sdrm_cod_grupo_dim cg
  ON cg.cod_grupo = COALESCE(ppd.am_cod_grupo, pe_stg.cod_grupo)
