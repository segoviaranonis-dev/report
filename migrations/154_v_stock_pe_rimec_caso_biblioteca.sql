-- MIG-154 — v_stock_pe_rimec lee descp_caso_snapshot (badge PROMO Web)
-- Tras MIG-153 vincular_biblioteca_a_pe

DROP VIEW IF EXISTS public.v_stock_pe_rimec;
...
SELECT
  ...
  COALESCE(
    ...
    ('PE · ' || COALESCE(pp.numero_proforma, ''::text))::text
  ) AS descp_caso,
...
COMMENT ON VIEW public.v_stock_pe_rimec IS
  'Pronta entrega RIMEC · PPD · MIG-154 descp_caso_snapshot biblioteca · badge PROMO Web';
