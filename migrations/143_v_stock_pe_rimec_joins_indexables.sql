-- MIG-143 — v_stock_pe_rimec: joins indexables (bigint) + índice PP PE
-- Problema: cast codigo_proveedor::text = ppd.linea → nested loop ×12k ×2243 líneas (~112s/80 filas)
-- Fix: cast ppd.text → bigint en LATERAL; join (proveedor_id, codigo_proveedor) indexable

CREATE INDEX IF NOT EXISTS idx_pp_pe_catalog
  ON public.pedido_proveedor (id)
  WHERE entidad_comercial = 'STOCK'
    AND deposito_codigo IS NOT NULL
    AND (estado_transito)::text = 'EN_DEPOSITO'
    AND categoria_id = 1
    AND quincena_arribo_id = 25;
...
COMMENT ON VIEW public.v_stock_pe_rimec IS
  'Pronta entrega RIMEC · PPD Alejandro Magno MIG-140/142/143 · joins indexables bigint';
