-- MIG-176 — CP tránsito: LPN/LPC/caso solo desde PPD vinculado (unidad de dirección)
-- Motor/listado = unidad de mando · cambio estrategia = re-vincular · Web/FI nunca leen precio_lista vivo
...
SELECT DISTINCT ON (ppd.id)
  ...
  COALESCE(ppd.precio_lpn, 0::numeric) AS lpn,
  ...
  COALESCE(NULLIF(btrim(ppd.descp_caso_snapshot), ''), '—'::text) AS descp_caso,
...
COMMENT ON VIEW public.v_stock_rimec IS
  'Catálogo RIMEC Web CP · MIG-176 precio solo PPD vinculado (sin fallback precio_lista vivo)';
