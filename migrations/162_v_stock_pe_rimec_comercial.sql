-- MIG-162 · v_stock_pe_rimec + comercial SDRM (COD.GRUPO · LIQUIDACIÓN)
-- Join sdrm_articulo_comercial + sdrm_cod_grupo_dim vía staging codigo_barras
...
SELECT
  ...
  COALESCE(sac.es_liquidacion, cg.es_liquidacion, false) AS es_liquidacion,
  COALESCE(sac.es_promo, cg.es_promo, false) AS es_promo,
  COALESCE(sac.cadena_comercial, cg.cadena_comercial, 'REGULAR'::text) AS cadena_comercial
...
COMMENT ON VIEW public.v_stock_pe_rimec IS
  'Pronta entrega RIMEC · PPD · MIG-162 comercial SDRM (COD.GRUPO · LIQUIDACIÓN)';
