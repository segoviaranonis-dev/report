...
export async function getFacturas(idCl?: number | null): Promise<FacturaListItem[]> {
  ...
  const { rows } = await pool.query<FacturaListItem>(
    `
    SELECT factura, factura_legacy, pedido, proforma, marca, fecha::text AS fecha,
    ...
    FROM (
      ...
      GROUP BY fi.pv_global, fi.nro_factura, pp.numero_registro, pp.numero_proforma,
               mv.descp_marca, fi.cliente_id, cv.descp_cliente, cl.numero_registro, cl.id, fi.created_at
    ) u
  ...
  return rows.map((r) => ({ ...r, pares: num(r.pares), traspaso_id: r.traspaso_id != null ? num(r.traspaso_id) : null }));
}
