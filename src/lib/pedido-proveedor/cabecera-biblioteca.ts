...
import { tryLockPpFiOps, unlockPpFiOps } from "./pp-fi-advisory-lock";
...
export async function cambiarBibliotecaPp(
  ...
  try {
    if (!(await tryLockPpFiOps(client, ppId))) {
      return {
        ok: false,
        error:
          "Hay un lote de facturas internas en curso en este PP — esperá a que termine y volvé a cambiar la biblioteca.",
      };
    }
    ppFiLocked = true;
