      ...
      <div className="grid shrink-0 grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="flex gap-4 lg:col-span-2">
          {top3.map((v, i) => (
            <div
              key={v.vendedor}
              className={`group relative flex-1 rounded-2xl border bg-white p-6 backdrop-blur-md transition-all duration-500 ${
                i === 0
                  ? "-mb-4 -mt-4 mb-4 border-rimec-azul/50 shadow-[0_0_30px_rgba(0,43,78,0.2)]"
                  : "mt-4 -mb-4 border-rimec-azul/15"
              }`}
            >
              <div className="absolute right-4 top-4 font-serif text-4xl italic opacity-20">#{i + 1}</div>
              <h4 className="mb-1 font-sans text-sm uppercase tracking-widest text-neutral-ink-muted">Vendedor</h4>
              <p className={`mb-6 truncate font-serif text-xl ${i === 0 ? "text-rimec-azul" : "text-neutral-ink"}`}>{v.vendedor}</p>

              <div className="space-y-4">
                <div>
                  <div className="mb-1 text-[10px] uppercase tracking-wider text-neutral-ink-muted">Volumen generado</div>
                  <div className="font-serif text-2xl tabular-nums">{fmtGs(v.monto_2026)}</div>
                </div>
                <div>
                  <div className="mb-1 text-[10px] uppercase tracking-wider text-neutral-ink-muted">Cumplimiento</div>
                  <div className="flex items-center gap-2">
                    <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-rimec-azul/5">
                      <div
                        className={`h-full ${v.cumplimiento_pct >= 100 ? "bg-semantic-success" : "bg-rimec-azul"}`}
        ...
        <div className="relative flex flex-col items-center justify-center rounded-2xl border border-rimec-azul/15 bg-white p-6 backdrop-blur-md">
          <h3 className="absolute left-6 top-6 font-serif text-sm uppercase tracking-widest text-rimec-azul/80">Ritmo global</h3>
          <div className="relative mt-8 h-40 w-full">
            ...
            <div className="absolute bottom-0 left-0 w-full text-center">
              ...
              <div className="mt-1 text-[10px] uppercase tracking-wider text-neutral-ink-muted">del objetivo general</div>
            </div>
          </div>
        </div>
      </div>
              ...
              {ranking_vendedores.map((v, rowIdx) => {
                ...
                return (
                  <tr key={`${v.vendedor}-${rowIdx}`} className="border-b border-rimec-azul/10 transition-colors hover:bg-white">
                    ...
                    <td className="px-6 py-3 text-right tabular-nums text-neutral-ink-muted">{fmtGs(v.objetivo)}</td>
                    <td className="px-6 py-3 text-right tabular-nums text-rimec-azul">{fmtGs(v.monto_2026)}</td>
