...
export default async function StockTransitoDisponiblePage() {
  const resumen = isRimecDatabaseConfigured()
    ? await getStockTransitoResumen(getRimecPool())
    : EMPTY_RESUMEN;

  return (
    <DepositoRimecShell footer="Saldo disponible · Compra previa tránsito · Panel Alejandro Magno">
      <StockTransitoClient resumenInicial={resumen} vista="disponible" />
    </DepositoRimecShell>
  );
}
