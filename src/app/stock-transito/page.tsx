...
export default async function StockTransitoPage() {
  const resumen = isRimecDatabaseConfigured()
    ? await getStockTransitoResumen(getRimecPool())
    : EMPTY_RESUMEN;

  return (
    <DepositoRimecShell footer="Stock Tránsito · Compra previa · filtro Llegada · Alejandro Magno">
      <StockTransitoClient resumenInicial={resumen} />
    </DepositoRimecShell>
  );
}
