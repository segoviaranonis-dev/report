...
import { useStockTransito } from "./StockTransitoContext";
...
function KpiCard({ label, value, sub }: { label: string; value: string; sub?: string }) {
  return (
    <div className="rounded-xl border border-report-rule bg-white p-3 shadow-sm">
      ...
      <p className="mt-1 text-lg font-black tabular-nums text-rimec-azul">{value}</p>
      {sub ? <p className="mt-0.5 text-[11px] text-report-muted">{sub}</p> : null}
    </div>
  );
}

export function TabArticulosTransito() {
  const {
    loading,
    err,
    drill,
  ...
  const ente = "Compra previa · tránsito";
  ...
  return (
    <div className="space-y-4 pb-6">
      ...
      <p className="text-xs text-report-muted">
        Vista filtrada ·{" "}
        <span className="font-bold tabular-nums text-rimec-azul">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
        ...
        <KpiCard label="Inicial vista" value={`${totalInicial.toLocaleString("es-PY")} p`} sub="comprado" />
      ...
      <DepositoChartSection
        index={1}
        title={`${ente} · Marcas`}
        subtitle="Compra previa tránsito · distribución por marca"
        slices={porMarca}
        totalPares={totalPares}
        defaultOpen
      />
